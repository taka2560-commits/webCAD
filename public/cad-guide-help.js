...
const GUIDE_TOPICS = [
    ...
    { id: 'coord', cat: 'start', title: '座標を読む（ルーペ・座標読取モード）', tour: 'fullscreen', keys: 'ざひょう よむ るーぺ かくだいきょう ざひょうよみとり 座標読取 全画面',
        ...
        tips: ['座標の文字の大きさ・桁・単位（m／mm）は、オプションの「表示・操作」で変えられます。'] },
    ...
    { id: 'measure', cat: 'dim', title: '測る（基点測定）', tour: 'measure', keys: 'はかる そくてい きてん きょり 距離 測定 基点',
        ...
        tips: ['距離は m なら小数3桁、mm なら整数で出ます。単位はオプションの「長さの単位」で選べます。'] },
    { id: 'dims', cat: 'dim', title: '寸法を記入する', keys: 'すんぽう へいこう せいれつ はんけい ちょっけい かくど ざひょうすんぽう れんぞく 寸法 角度 直径 半径 連続',
        steps: [
            ...
            '∠角度: 角の頂点 → 1本目の辺の上の点 → 2本目の辺の上の点（度か度分秒かは、オプションの「角度の表示」）。',
            'XY座標: 点 → 文字の位置（引出線で X・Y を書き込みます）。',
            '📏連続寸法（座標読取モードの道具箱、コマンド DIMCONT）: 点を続けて指定して、並べて記入します。',
        ],
        tips: [{ touch: '寸法の点は、指を離したあと ☑確定 で決まります。', pc: '寸法の点はクリックで決まります。' }, '寸法の文字の大きさ・桁と、長さの単位（m／mm）は、オプションの「表示・操作」で変えられます。'] },
    ...
    { id: 'helm', cat: 'survey', title: 'SIMA の座標変換（ヘルマート変換）', tour: 'helm', where: '⋯ メニュー → 🧮 測量計算 → 変換', keys: 'へんかん へるまーと はりあわせ ざひょうへんかん しま かりざひょう 変換 張り合わせ 座標変換 仮座標',
        ...
        tips: ['ほかの組と合わない組（点の取り違えの疑い）と、張り合わせ点で囲んだ範囲の外（外挿）の点は、黄色で知らせます。',
            ...
            '別窓の □ で画面いっぱいに広げると、点を大きく見てタップできます（図面でなぞるあいだはたたみ、☑確定のあと広げ直します。❐ で元の大きさ）。'],
    ...
    { id: 'panel', cat: 'setting', title: 'パネルの移動・たたむ・画面いっぱい', tour: 'panel',
        keys: 'ぱねる いどう うごかす ずらす たたむ ひろげる かくだい さいしょう さいだいか おおきく がめんいっぱい べつまど パネル 移動 拡大 最小 最大化 別窓 じゃま 邪魔',
        steps: ['パネルの見出し（⠿ の帯）をつまむと動かせます。置いた場所は次も覚えています。', '見出しをダブルタップすると、画面の中央に戻ります。',
            '見出しの ▁ で見出しだけにたたみます（図面を広く見たいとき）。もう一度押すと広げます。',
            '□ で画面いっぱい（上のバーと下のバーのあいだ）に広げ、❐ で元の大きさ・位置に戻します。表や一覧を大きく見られます。タブを切り替えても広げたままで、✕ で閉じると元に戻ります。',
            '別窓（変換の SIMA）の見出しにも ▁・□ があります。別窓は右下のつまみでも大きさを変えられ、「全体」で点をすべて表示します。'],
        tips: ['画面いっぱいのパネル・別窓で、図面の点を指定するときは、指定のあいだだけパネルを隠し（別窓はたたみ）、終わると元に戻します。'] },
    { id: 'prefs', cat: 'setting', title: '表示の設定（ルーペ・文字・吸着）', tour: 'prefs', keys: 'ひょうじ せってい るーぺ もじ おおきさ けた きゅうちゃく 表示 設定 文字 桁 吸着',
        steps: ['オプションの「表示・操作」で、ルーペの大きさ・倍率、座標の文字の大きさ・桁・単位、寸法の文字と桁、長さの単位、角度の表示（度・度分秒）、吸着の範囲、ボタンの大きさ、屋外モードを変えられます（端末ごと）。'],
        tips: ['単位（m・mm）・角度の詳しい説明は「表示の単位（m・mm）」「角度の表示（度・度分秒）」、ボタンの大きさ・屋外モードは「現場で使いやすく」を見てください。'] },
    { id: 'units', cat: 'setting', title: '表示の単位（m・mm）', keys: 'たんい めーとる みりめーとる みり m mm 単位 メートル ミリ 座標 長さ 寸法 換算',
        lead: '座標と長さ（寸法・測定・半径・距離など）を、m か mm で表示・入力できます。',
        where: '⋯ メニュー → ⚙ オプション → 表示・操作 の「座標の単位」「長さの単位」',
        steps: [
            '「図面どおり」（初期値）は、図面の1単位（オプションの測量の欄: 1m か 1mm）のまま出します。m の図面は小数3桁まで、mm の図面は整数（どちらも 1mm まで）です。',
            'DWG・DXF を開くと、ファイルに書かれた単位（mm・m）に「図面の1単位」を自動で合わせます（今の図面に追加したときは変えずに知らせます）。',
            '「m」か「mm」を選ぶと、その単位に直して出します。座標: ステータスバー・ルーペ・座標寸法・プロパティ。長さ: 寸法・測定・半径・距離・間隔。',
            '数を入れるときも同じ単位です（コマンド欄の X,Y と @X,Y、円の半径・長方形・文字の高さ・オフセット・配列の間隔・角の処理・相対入力・プロパティ欄）。',
            '座標と長さは別々に選べます。例: 1単位＝1m の図面で、座標は m のまま、寸法だけ mm にする。',
            '⭐ お気に入りに「長さ単位」「座標単位」のボタンを登録すると、押すたびに m ⇔ mm を切り替えます（ボタンにいまの単位が出ます。コマンド UNIT・CUNIT でも同じ）。',
        ],
        tips: ['単位を選ぶと、入力欄の見出しに (mm) のように単位が付きます。', '記入した寸法（基点測定の寸法も）は、単位を変えるとすぐ描き直します。文字を書き換えた寸法はそのままです。', '切り替えのボタンで図面の1単位と同じ単位に戻すと「図面どおり」になります（2回押すと元どおり）。', '測量計算・杭打ち・SIMA・座標一覧は、これまでどおり m です。'],
        open: { label: '⚙ オプションを開く', run: () => showOptionsPanel() } },
    { id: 'angles', cat: 'setting', title: '角度の表示（度・度分秒）', keys: 'かくど どぶんびょう ど ふん びょう 角度 度分秒 秒 回転 角度寸法',
        lead: '角度寸法・回転の記録・UCS の角度を、度（45.5°）か度分秒（45°30′00″）で出します。',
        where: '⋯ メニュー → ⚙ オプション → 表示・操作 の「角度の表示」',
        steps: [
            '「度」（初期値）は 45.5° のように、角度寸法は小数1桁で出します（これまでと同じ）。',
            '「度分秒」は 45°30′15″ のように、秒まで出します。記入済みの角度寸法も、すぐ描き直します。',
            '度分秒にすると、回転の「角度」の欄に「45 30 15」「45-30-15」のように度 分 秒で入れられます（45.5 のような度の数も入ります。先頭に - で右回り）。',
        ],
        tips: ['測量計算・杭打ちの方向角・夾角は、これまでどおり度分秒です。', 'DXF・印刷に書き出す角度寸法の文字も、同じ表示になります。'],
        open: { label: '⚙ オプションを開く', run: () => showOptionsPanel() } },
    { id: 'options', cat: 'setting', title: 'オプション（背景色・単位・系番号など）', keys: 'おぷしょん せってい はいけい たんい けいばんごう えらーろぐ 背景 単位 系番号 エラーログ',
        steps: [
            '背景色（黒・グレー・白）。',
            '表示・操作: ルーペ・座標の文字と桁と単位・寸法の文字と桁・長さの単位（m／mm）・角度の表示（度／度分秒）・吸着の範囲・ボタンの大きさ・屋外モード。',
            '測量: 図面の1単位（1m／1mm。DWG・DXF を開くと、ファイルの単位に自動で合わせます）、現在地の系番号。',
            'タップでブロック全体を選択、取り込み時に円弧を非表示、DWG・PDF の読込エンジンを端末に保存、⚠ エラーログ。',
        ],
        open: { label: '⚙ オプションを開く', run: () => showOptionsPanel() } },
    { id: 'field', cat: 'setting', title: '現場で使いやすく（屋外モード・ボタンの大きさ）', keys: 'おくがい ひなた まぶしい みえにくい てぶくろ ぼたん おおきさ 屋外 日なた 手袋 ボタン 大きさ 見やすく 太く',
        lead: '日なたでも図面を読みやすく、手袋のままでもボタンを押しやすくします。',
        where: '⋯ メニュー → ⚙ オプション → 表示・操作 の「ボタンの大きさ」「屋外モード」',
        steps: [
            '屋外モード「入」: 図形・寸法の線を太く、文字を太字にして背景色で縁取り、背景（黒・グレー・白）との明るさの差が小さい色を明るく・暗くします。パネルやバーは不透明にして、文字を明るく・太くします。',
            'ボタンの大きさ（小・中・大・特大）: 上・左・下のバー、選んだときのバー、☑確定のバー、お気に入りのバー、右下の ？・コマンドのボタンを大きくします。⋯ メニューとパネルの大きなボタンも少し高くなります。',
            '⭐ お気に入りに「屋外」のボタンを登録すると、押すたびに屋外モードを入・切できます（入のあいだは光ります。コマンド OUTDOOR でも同じ）。',
        ],
        tips: ['日なたでは、オプションの背景色を「白」にするのも効果があります（白い線は黒で描きます）。', 'ボタンを大きくすると、左のツールバーは中でスクロールして全部のボタンを出します。'],
        open: { label: '⚙ オプションを開く', run: () => showOptionsPanel() } },
    ...
    // ---- 困ったとき ----
    { id: 'faq-sun', cat: 'faq', title: '日なたで見えにくい・手袋で押しにくい', keys: 'ひなた まぶしい みえにくい てぶくろ おしにくい 屋外 手袋',
        steps: [
            'オプションの「屋外モード」を「入」にすると、線が太く、文字が太字で縁取られ、パネルが不透明になります。',
            'オプションの「ボタンの大きさ」を「大」「特大」にすると、バーのボタンが大きくなります。',
            '背景色を「白」にすると、日なたで見やすくなることがあります。',
        ],
        open: { label: '⚙ オプションを開く', run: () => showOptionsPanel() } },
    ...
    { id: 'faq-coord', cat: 'faq', title: '座標が合わない・ずれて見える', keys: 'ざひょう あわない ずれる たんい けいばんごう',
        steps: [
            ...
            '図面の1単位（1m／1mm）がファイルと合っているか、オプションで確かめます（DWG・DXF にファイルの単位が書かれていれば、開いたときに自動で合わせます）。座標が 29510.405 のように細かすぎるときは、mm の図面が 1m のままになっています。',
...
const GUIDE_COMMANDS = [
    ...
    ['図面・表示', [['OPEN', '-', '開く（IMPORT）'], ['SAVE', '-', '保存'], ['PROJECTS', '-', '保存一覧（RESTORE）'], ['EXPORTDXF', 'SAVEAS', 'DXF出力'], ['PRINT', 'PDF', '印刷・PDF（PLOT）'],
        ...
        ['BLOCKS', '-', 'ブロック管理'], ['LAYOFF', '-', 'タッチで画層を非表示'], ['SHOWALL', '-', '隠した図形を再表示'], ['FAV', 'OKINI', 'お気に入りの登録'], ['UNIT', 'LENUNIT', '長さの単位を m ⇔ mm に切り替え'], ['CUNIT', 'COORDUNIT', '座標の単位を m ⇔ mm に切り替え'], ['OUTDOOR', 'SUN', '屋外モードの入・切'], ['ERRORS', 'ERRLOG', 'エラーログ']]],
];
