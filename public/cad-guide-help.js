...
const GUIDE_TOPICS = [
    ...
    { id: 'prefs', cat: 'setting', title: '表示の設定（ルーペ・文字・吸着）', tour: 'prefs', keys: 'ひょうじ せってい るーぺ もじ おおきさ けた きゅうちゃく 表示 設定 文字 桁 吸着',
        steps: ['オプションの「表示・操作」で、ルーペの大きさ・倍率、座標の文字の大きさ・桁・単位、寸法の文字と桁、長さの単位、角度の表示（度・度分秒）、吸着の範囲を変えられます（端末ごと）。'],
        tips: ['単位（m・mm）・角度の詳しい説明は「表示の単位（m・mm）」「角度の表示（度・度分秒）」を見てください。'] },
    ...
    { id: 'options', cat: 'setting', title: 'オプション（背景色・単位・系番号など）', keys: 'おぷしょん せってい はいけい たんい けいばんごう えらーろぐ 背景 単位 系番号 エラーログ',
        steps: [
            '背景色（黒・グレー・白）。',
            '表示・操作: ルーペ・座標の文字と桁と単位・寸法の文字と桁・長さの単位（m／mm）・角度の表示（度／度分秒）・吸着の範囲。',
...
const GUIDE_COMMANDS = [
    ...
    ['図面・表示', [['OPEN', '-', '開く（IMPORT）'], ['SAVE', '-', '保存'], ['PROJECTS', '-', '保存一覧（RESTORE）'], ['EXPORTDXF', 'SAVEAS', 'DXF出力'], ['PRINT', 'PDF', '印刷・PDF（PLOT）'],
        ...
        ['BLOCKS', '-', 'ブロック管理'], ['LAYOFF', '-', 'タッチで画層を非表示'], ['SHOWALL', '-', '隠した図形を再表示'], ['FAV', 'OKINI', 'お気に入りの登録'], ['UNIT', 'LENUNIT', '長さの単位を m ⇔ mm に切り替え'], ['CUNIT', 'COORDUNIT', '座標の単位を m ⇔ mm に切り替え'], ['ERRORS', 'ERRLOG', 'エラーログ']]],
];
