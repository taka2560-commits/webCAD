...
const GUIDE_HINTS = [
    ...
    { key: 'MIRROR', title: '⇋ 鏡像', prefix: 'WAITING_MIRROR_', steps: [
        { modes: ['SELECT'], text: '映す図形をタップ（範囲選択もできます）' }, { modes: ['P1'], text: '鏡の線の1点目をタップ' },
        { modes: ['P2'], text: '鏡の線の2点目をタップ（元の図形を残すかは、パネルで選びます）' }] },
    { key: 'SCALE', title: '⤢ 尺度変更', prefix: 'WAITING_SCALE_', steps: [
        { modes: ['SELECT'], text: '大きさを変える図形をタップ' }, { modes: ['BASE'], text: '基点（動かない点）をタップ。倍率はパネルで（長さからも求められます）' }] },
    { key: 'ARRAY', title: '▦ 配列', prefix: 'WAITING_ARRAY_', steps: [
        { modes: ['SELECT'], text: '並べる図形をタップ' },
        { modes: ['SET'], text: 'パネルで数と間隔を決めて ☑確定（円形に並べるときは、先に中心をタップ）' }] },
    { key: 'BREAK', title: '÷ 分割', prefix: 'WAITING_BREAK_', steps: [
        { modes: ['SELECT'], text: '分ける図形をタップ（「等分する」なら、タップだけで分けます）' },
        { modes: ['POINT'], text: '分ける点をタップ（交点・端点などに吸い付きます）' }] },
    { key: 'JOIN', title: '⋈ 結合', prefix: 'WAITING_JOIN_', steps: [
        { modes: ['SELECT'], text: 'つなぐ線をタップ（つながった線をまとめて選びます。もう一度タップで外す）。☑確定 で1本にします' }] },
    { key: 'FILLET', title: '╭ 角の処理', prefix: 'WAITING_FILLET_', steps: [
        { modes: ['SELECT'], text: '1本目の線の、残す側をタップ（ポリライン・長方形の角の面取りは、角をタップ）' },
        { modes: ['SECOND'], text: '2本目の線の、残す側をタップ' }] },
    { key: 'GRIP', title: '✥ 点を動かす', prefix: 'WAITING_GRIP_', steps: [
        { modes: ['DEST'], text: { touch: '動かす先までなぞって指を離します（緑の記号＝スナップ）。やめるときは ❌終了', pc: '動かす先をクリック、または押したまま動かして離します（スナップが使えます）。Esc でやめる' } }] },
...
const GUIDE_COMMANDS = [
    ...
    ['COORDS', 'ZAHYO', '座標一覧'], ['COGO', 'CALC', '測量計算'], ['AREA', 'AA', '求積'], ['INV', '-', '逆計算'], ['TS', 'SOKKIA', 'TS連携'], ['SDROUT', '-', 'SDR33出力'], ['STAKE', 'KUI', '杭打ち'], ['PRINT', 'PDF', '印刷・PDF'], ['MAP', 'SHITAE', '地図・下絵'], ['PHOTO', 'MEMO', '写真・メモ'], ['SIMAOUT', '-', 'SIMA出力'], ['CSVOUT', '-', '座標CSV出力'], ['GNSS', 'GPS', '現在地'], ['UCS', '-', 'UCS（原点）'], ['UCS2P', '-', 'UCS（2点）'],
