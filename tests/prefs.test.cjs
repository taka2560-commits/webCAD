...
describe('表示・操作の設定', () => {
    ...
    it('初期値に戻すと、すべて以前と同じ値になる', () => {
        ...
        assert.equal(active, 'angleFormat=deg,btnSize=m,coordDecimals=std,coordFont=m,coordUnit=auto,dimDecimals=auto,dimText=m,lenUnit=auto,loupeSize=m,loupeZoom=3,outdoor=off,snapRange=m');
    });
